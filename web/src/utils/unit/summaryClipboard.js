...
export function padStartNbsp(value, width) {
  return NBSP.repeat(Math.max(0, width - value.length)) + value;
}

export function padEndNbsp(value, width) {
...
export function buildSummaryPlainText(rows, idWidth, nomeWidth, aneelWidth) {
  return rows
...
export function buildSummaryHtml(rows, idWidth, nomeWidth, aneelWidth) {
